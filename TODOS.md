...
> **資料基準（2026-09-22）：** `storiesByNewest()` **31 集**、最新 **`ep-31`**（2026-09-22，MVP 單封面）。
> **出圖落後：** 多頁插圖至 **`ep-29`（15 頁，`77429838`）**；`ep-25`／`ep-26`／`ep-27`／`ep-28`／`ep-30`／`ep-31` 仍為 `pageCount=1` MVP。最新全幕含 **`ep-21`（18 頁）**／**`ep-22`（24 頁）**／**`ep-23`（26 頁）**／**`ep-24`（19 頁）**／**`ep-29`（15 頁）**。
> 完整逐字稿覆蓋至 `ep-31`（ep-1–30 已 `--mark`；`ep-31` 中秋烤肉已校對；`ep-30` 已手切 18 幕，待出圖 `8ad9df05`）。`data/games.ts` 見下表。
>
...
> **2026-09-20 實測（`6a7f6741`）：** 390×664 方塊格子 15→25.9px、井底 652 同屏；360×740 17→24.2px（寬度受限）；390×844 22→27.2px。消消樂 `#game-play` 684→480px。著色本 390×664／1280×800 畫布可見時色盤與工具列皆可 `elementFromPoint` 命中。

| ID | 級 | 項目 | 證據／根因 | 修法方向 | 位置 | 工時 | 狀態 |
|----|----|------|-----------|---------|------|------|------|
| G-C1 | C | **消消樂「暫停」壞掉**：按下後暫停鈕直接消失、沒有「繼續」，棋盤照常可點（按完暫停仍能消到 1/4） | controller 註冊 effect 的 deps 含 `startLevel` ← `armIdleHint` ← `inputPaused` ← host `status`；status 變 `paused` 就重跑 effect，呼叫 `instance.notifyReady("title")` 把 status 打回 `ready`（順帶關 BGM、`useVisibilityPause` 失效） | controller 用 ref 註冊；`notifyReady("title")` 只在 mount 跑一次；補回歸測試「pause 後 toolbar 有『繼續遊戲』且棋盤 disabled」 | `components/games/CandyMatchView.tsx` L333–349、L239–252；`lib/gamekit/games/candy-match/adapter.ts` | S | ✅ `6a7f6741` |
| G-H1 | H | **方塊井在真實手機小到不能玩**：390×664 井寬 150px、格子 **15px**；360×740 是 17px；390×844 也只有 22px；桌機 1280×800 井底 824 > 800 要捲 | 井上方吃掉 ~380px：sticky 抬頭 84（≤430 標題換行）＋卡內「繽紛方塊」標題列＋難度 chip＋HUD 130＋教學橫幅 70；`boardScale` 用剩餘高度縮放、下限 300px | 局內 HUD 壓成單列（分數·Lv·下一個）、卡內標題拿掉（h1 已在抬頭）、教學橫幅改浮在井上的 toast、抬頭局內不換行；驗收：390×664 格子 ≥26px、井底＋觸控鍵同屏 | `components/games/BlockDropView.tsx` L1023–1036（縮放）、L88–118（layout metrics）；`components/games/GamePageShell.module.css` `.playHeader`／≤430 換行規則 | M | ✅ `6a7f6741` |
| G-H2 | H | **方塊觸控左右鍵只有 29px 寬**（旋轉／落下 64px），6–12 歲拇指難按；DESIGN 觸控 ≥44px 紅線 | 側欄 pad `colW: 64` 內塞兩顆 | 左右各 64px 獨立一列，或改井下方橫列；`TouchControls.test.tsx` 補 ≥44 斷言 | `components/games/BlockDropView.tsx` `getLayoutMetrics` `touch` | S | ✅ `6a7f6741` |
...
| G-M7 | M | 著色本 chrome 不一致：只有它保留全站 nav，還疊「← 回遊樂園」「← 回封面」三層返回；桌機「← 回遊樂園」貼視窗左緣、內容置中不對齊 | `lib/is-story-play-route.ts` 把 `coloring-book` 排除在沉浸路由外（PLAY-IA-3 D5-A 決策「不動」） | 走 `GamePageShell` 同款 sticky 抬頭（返回＋h1＋主題），picker 的「回封面」併進抬頭 | `components/coloring/ColoringPageShell.tsx`、`lib/is-story-play-route.ts` | M | ✅ `6089fd07`（D5-A 已翻：著色本納入沉浸路由） |
| G-L1 | L | 方塊 Game Over 主 CTA 是「去玩：繽紛消消樂」，「再玩一次」變次要——挑戰型輸了應以重玩為主 | `GameEndStation` `mood="over"` 把 next-game 當 mainAction | `over` 時主鈕改 replay、下一站降為連結 | `components/games/GameEndStation.tsx` | S | ✅ `a9630107`（K-12 順帶） |
| G-L2 | L | 星星／車庫／貼紙經濟還在算（消消樂會給星），但 hub 進度列在 `a166997b` 改版時拿掉；`lib/games/hub-progress.ts` 只剩測試在用。孩子拿到星星沒地方看 | — | 二選一：hub 卡下方補一列低壓進度（星星／已玩／下一輛），或刪 `hub-progress.ts`＋測試（knip） | `app/games/page.tsx`、`lib/games/hub-progress.ts` | S | ✅ `a9630107`（K-4：刪） |
| G-L3 | L | 蠟筆不受線稿區域限制（會塗出界）；同齡產品常見「自動不出線」模式 | — | 以 flood-fill 區域當筆刷遮罩（可選開關） | `components/coloring/ColoringCanvas.tsx` 引擎 | M | ✅ `545fb370`（無開關，預設不出線） |
...
> **判準：** 小孩不看說明文字，也能從畫面看出「這是什麼遊戲」以及「下一步要按哪裡」。
> **接：** 結算站圖示分化＋下一站縮圖、卡片 56px 玩法 play 鈕＋三字動作詞（圖為主）、meta 只留年齡、方塊 ready 無字示範。
> **出圖才做：** hero 站牌／道路指向、卡片縮圖玩法動畫。**不做：** 站點亮起／貼紙（＝進度，K-4 已刪）、改名（孩子不讀名字）。

| ID | 項目 | 狀態 |
|----|------|------|
| K-13 | 結算站：下一關＝往前箭頭、換一張＝翻頁、再玩＝循環箭頭；去下一站＝下一款縮圖＋小箭頭 | ✅ `3e0a434a` |
...
> | M1 | 故事頁桌機三種欄寬、本集介紹卡懸中 | 已修：Plan `docs/plans/2026-09-17-story-desktop-intro-column.md`；介紹卡進左欄與 CTA 同軸、去左緣條 `9f9f3477` |
...
> | M5 | 手機故事列表 88px 縮圖＋三行摘要像目錄 | 已修：≤480 縮圖 120px、摘要 2 行 `fc0467a1` |
> | M6 | RelatedStories 與 StoryCard anatomy 不同 | 已修：相關／收藏／主題／車種列表一律 `hideMeta` `fc0467a1` |
> | M7 | 家長頁育兒小筆記左緣色條容器；統計格第二列變高 | 已修：callout 改 hairline＋elevated 卡；統計格刪「同步檢查」剩三格，桌機直向堆疊 `3f298882` |
> | M8 | `/subscribe` 登記關閉時是空殼 | 已修：關閉時四平台大按鈕列成主內容 `fc0467a1` |
> | L1 | 頁尾 ConnectHub 390 折 3+1；分隔點孤懸行尾 | 已修：≤480 4 欄 grid；分隔點改 `.metaLegal::before`，折行時不畫 `3f298882` |
> | L2 | 播放器「AI」鈕無說明；主播放鈕不夠主 | 已修：`--play-size` 大一階、圖示改 Aa `fc0467a1` |
...
> 決策 D0=C（桌機常駐兒童三入口、家長項收抽屜）／D1=A／D2=A／D5=A／D6=A／D7=B／D8=B／D9=A／D10=A。
...
- [ ] **PR2 後續（設計審建議）**　`nav · S · PR2`
  ...
  - (5) ~~睡前模式 veil 可能蓋掉磁貼標題對比~~ **已實測排除（2026-08-30）**：線上正式站 `data-theme=night`＋`data-bedtime=true`，並把探索區推到視窗頂端 60px（veil 漸層最深的 36–38% 正下方）量測合成像素——磁貼標籤 **7.22:1**、地圖大卡標籤 **6.94:1**、區塊標題 **6.25:1**（正常捲動位置），門檻 4.5:1，最低仍有 1.6 倍餘裕。
...
> | 版本 | Performance | LCP | CLS | a11y | SEO |
> |------|------------|-----|-----|------|-----|
> | `7e70c1c`（改版前） | **78** | 6.0s | 0 | 100 | 100 |
> | `d5bccab`（PR1+PR2） | **78** | 6.1s | 0 | 100 | 100 |
> | 線上 `podcast-website-mu.vercel.app` | 78–81 | 4.8–5.8s | 0 | 100 | 100（BP 100） |
>
> 結論：本次改版**效能中性**（Performance 中位數持平、LCP +0.1s 在雜訊內、CLS 皆 0）。
> 但外部清單的絕對門檻（Perf ≥85、LCP ≤2.5s）**在改版前的 main 上就已不達標**，
> 因此依 Approved Plan 改用相對門檻：**不得劣於 baseline**。要真正拉到 85／2.5s
> 是另一件事（四張滿版 hero 的 LCP 工程），不屬本輪 scope。
> 注意：本機 `npm start` ＋模擬 4G 節流與 Vercel CDN 不同環境，數字不可互相比較，
> 只可各自比對同環境的 before/after。
...
| content(subtitles): ep-26 中元節字幕人工校對（噗噗豬／海龜老師暖暖／吉普車正名、刪 3 句重複，167 → 164 句 `--mark`） | 見本 commit |
| content(subtitles): ep-27 小紅豆上學字幕人工校對（小鹿被被／片頭 jingle／小紅豆正名、支持、笑得、讚、代名詞，拆「小紅豆／媽媽」，178 → 179 句 `--mark`） | 見本 commit |
...
| docs/data(play-map): A4 最終輪 12 筆 tips；STOP editorial cleanup | `47d0598` |
| docs/data(play-map): B1 Phase 2 嘉義市新增嘉義公園 | `20e2e85` |
| feat(play-map): B1 Phase 3 上線嘉義市合輯 `chiayi-city` | `cd5544d` |
...
| LIST-2 後續 文案誠實化 + e2e | ✅ `289fbc4` | 對齊「只收名單、不寄新集上線信」；確認信僅驗證信箱；SubscribeForm Playwright；ESP 另案 | `lib/subscribe-copy.ts`、`SubscribeForm`、`/subscribe`、`/legal`、`DISCLAIMER.md`、`e2e/subscribe.spec.ts` | `npm test` + `e2e/subscribe.spec.ts` | `289fbc4` |
...
> **2026-09-20 修訂：** D5-A 已翻（G-M7 `6089fd07`）——著色本改走同款 sticky 抬頭並納入沉浸路由；D1-A 的操作提示改以圖代字（K-9），位置契約不變。
...
| UX-P1-5 | P1 | ✅ `99cf197` | **e2e 兒童 UX 回歸**：`/adventures` a11y + 地圖觸控；遊樂園 hub；`/for-parents` 與播放頁觸控 ≥44px；CI `e2e-child-path` | `e2e/a11y.spec.ts`、`e2e/universe-map.spec.ts`、`e2e/games.spec.ts`、`e2e/child-ux.spec.ts` | `npm run test:e2e:ci` |
...
### Playwright E2E CI 接入 + 擴充　`P3 · M · 無`　〔eng〕　✅ `99cf197`
