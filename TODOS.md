...
- [ ] **PR2 後續（設計審建議）**　`nav · S · PR2`
  - **(1) 觸發器文字「選單」— 維持現狀（2026-08-30 使用者裁決 A）。** 對 3–7 歲仍是抽象詞，但**現在不能改「給爸媽」**：抽屜 9 列裡 6 列是兒童項（首頁／全部故事／角色圖鑑／遊樂園／繪本著色／宇宙地圖），只有 3 列是家長項，標成「給爸媽」是標籤說謊。正解分兩步：先讓兒童完全不需要這顆鈕（把角色圖鑑／繪本著色／宇宙地圖移出抽屜、交給磁貼牆承接），抽屜縮為純家長清單後**再一次**改標籤＋`aria-label`，同批更新 DESIGN §99／§276 與四支 e2e。
    **⚠️ 執行前的硬前提：`ExploreGrid` 磁貼牆只在首頁。** 內頁（`/stories`、`/games`、`/story/*` 等）沒有替代入口，若先把兒童項從抽屜拿掉，這三個入口會在所有內頁**徹底消失**——除非先讓等價入口出現在每一頁。那是另一個規模的工程，不是本項的 S。
  - **(2) 「留言」移入抽屜 — ✅ 已完成**（使用者裁決 B，見 `b9de774`）。決策關鍵是實測首頁曾同時有三個指向同一信箱的入口。
