...
`lib/gamekit/chiptune-bgm` 四款程序生成循環 BGM；`GameKitAudioBus` music／sfx 分軌；擴充 `useGameAudio(gameId)`（`playBgm`／`pauseBgm`／`stopBgm`）；四款遊戲接入，分頁隱藏暫停 BGM。**Phase 4 待做**：juice 工具組。

### Game Kit Phase 4 — juice　`P3 · L · Phase 3 ✅`　〔eng+design〕
粒子/震動/頓幀/緩動工具組逐款套用（`lib/gamekit/juice`）。
