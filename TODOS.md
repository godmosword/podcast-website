...
- [ ] **PR2 後續（設計審建議）**　`nav · S · PR2`
  - **(1) 觸發器文字「選單」— 維持現狀（2026-08-30 使用者裁決 A）。** 對 3–7 歲仍是抽象詞，但**現在不能改「給爸媽」**：抽屜 7 列裡 5 列是兒童項（全部故事／角色圖鑑／遊樂園／繪本著色／宇宙地圖），只有 2 列是家長項，標成「給爸媽」是標籤說謊。正解分兩步：先讓兒童完全不需要這顆鈕（把角色圖鑑／繪本著色／宇宙地圖移出抽屜、交給磁貼牆承接），抽屜縮為純家長清單後**再一次**改標籤＋`aria-label`，同批更新 DESIGN §99／§276 與四支 e2e。
...
| ID | 說明 |
|----|------|
| polish(nav): 頂欄分流 1=C 2=A 3=A | 見本 commit |
