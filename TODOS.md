...
| GEO-1 crawler-access | 完成，待 hash 回填 | AI crawler allowlist + `llms.txt`；未新增 optional `llms-full.txt`，避免摘要索引漂移 | Modify: `app/robots.ts`; Create: `public/llms.txt`; Test: `app/robots.test.ts`, `lib/llms-static.test.ts` | `npm test -- app/robots.test.ts lib/llms-static.test.ts` passed；`npm run build` passed；local `curl /robots.txt`、`curl /llms.txt` 皆 200；既有 `* Allow: /` 與 sitemap 保留 | 待回填 |
