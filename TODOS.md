...
| # | ID | 類型 | 工時 | 狀態 |
|---|-----|------|------|------|
| 1 | [UX-P0-1](#兒童-ux-與親子互動稽核2026-07-11) 家長閘門 | trust | M | ✅ `a824dd8` `304e6b9` |
| 2 | [UX-P1-5](#兒童-ux-與親子互動稽核2026-07-11) 全站 e2e | eng | S | ✅ `a824dd8` |
...
| ID | 優先 | 狀態 | 摘要 | 主要檔案 | 驗證 |
|----|------|------|------|----------|------|
| UX-P0-1 | P0 | ✅ `a824dd8` `304e6b9` | **家長閘門**：進 `/for-parents/dashboard` 前簡單算術題；session 通過後放行（決策 A：不含 GameKit 設定） | 新 `ParentGate.tsx`；`ParentDashboardScreen.tsx` | 手動 375px；`npm test`；`e2e/parent-gate.spec.ts` |
...
| UX-P1-1 | P1 | ✅ `a824dd8` | **補齊 &lt;44px 按鈕**（只改 CSS）：~~ZoneSheet 關閉／wishToggle~~ → MAP-UX-P1a；~~Landing 箭頭~~ → 44×44（本輪 Design Review）；StoryPlayer 定時選項 min-height 44px | 各 `*.module.css` | 手動＋e2e |
...
| UX-P1-4 | P1 | ✅ `a824dd8` | **播放進度條拇指加大**（CSS 變數，不動控制列 layout）：`--seek-hit: 44px` | `StoryPlayer.module.css` | 手動播放頁；`e2e/story-player-ux.spec.ts` |
| UX-P1-5 | P1 | ✅ `a824dd8` | **e2e 兒童 UX 回歸**：`/adventures` a11y + 地圖觸控 e2e ✅（MAP-UX-P1c）；遊樂園 hub + 五款遊戲頁首屏／heading／返回動線 ✅（PLAY-IA-1）；`/for-parents` a11y；播放頁 a11y＋44px；家長閘門鎖定／放行 | `e2e/a11y.spec.ts`、`e2e/universe-map.spec.ts`、`e2e/games.spec.ts`、`e2e/parent-gate.spec.ts`、`e2e/story-player-ux.spec.ts` | `npm run test:e2e` |
...
| UX-P2-2 | P2 | ✅ `a824dd8` | 儀表板「最佳分數」改低壓文案（「探索紀錄」） | `ParentDashboard.tsx` | 手動 |
| UX-P2-3 | P2 | ✅ `eb19c86` | 遊戲頁年齡標示一致（metadata vs chip）：五款稽核無矛盾；challenge 提示由 GameIntro 統一呈現 | `app/games/page.tsx` | build |
| UX-P2-4 | P2 | ✅ `a824dd8` | Dudu 鍵盤可及：互動夥伴已是真正 `<button>`（非內層 tabIndex） | `DuduMoment.tsx` | a11y 單測 |
| UX-P2-5 | P2 | ✅ `a824dd8` | `reflectionShown` 加 `source: detail \| end-screen` 精準量測；舊 string[] 視為 end-screen | `progress-store.ts`、`ReflectionPrompt.tsx` | test |
...
1. ~~**UX-P0-2 + UX-P0-3**~~ ✅ `964f418`（W27-2）→ ~~**MAP-UX-P1a/b/c／P2a**~~ ✅ `80457c4` → ~~**UX-P1-5**~~ ✅ `a824dd8`
2. ~~**UX-P0-1**~~ ✅ `a824dd8` `304e6b9`（決策 A：僅 dashboard）
3. ~~**UX-P1-1**／**UX-P1-4**~~ ✅ `a824dd8`
4. **UX-P1-3** 內容分批，不擋工程項
5. **UX-P2-6** 擇機（生圖）；其餘 P2 ✅ `a824dd8`
...
用星星／笑臉呈現「聽了哪幾集、玩了什麼、解鎖了幾台車」，不做成績單。可先讀現有收藏／繼續播放／遊戲 best 分／圖鑑解鎖（localStorage）。**MVP 已上線**（`/for-parents/dashboard`）；文案 polish ✅ `a824dd8`（UX-P2-2）。

#### 家長閘門（parent gate）　`STEM-P3 · S · 付費／設定頁`　〔stem+eng〕
設定、未來付費頁前簡單算術題，防孩子誤觸（對齊 Sago Mini）。**✅ `a824dd8` `304e6b9`：** 先擋 `/for-parents/dashboard`（決策 A，不含 GameKit）。
