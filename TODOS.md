...
| LIST-2 後續 文案誠實化 + e2e | ✅ 見本 commit | 對齊「只收名單、不寄新集上線信」；確認信僅驗證信箱；SubscribeForm Playwright；ESP 另案 | `lib/subscribe-copy.ts`、`SubscribeForm`、`/subscribe`、`/legal`、`DISCLAIMER.md`、`e2e/subscribe.spec.ts` | `npm test` + `e2e/subscribe.spec.ts` | 見本 commit |
