...
> 完整逐字稿覆蓋至 `ep-30`（ep-1–10 已補 `--mark`；ep-11 起原本已標記；`ep-30` 已校對＋手切 18 幕，待出圖 `37cea402`）。`data/games.ts` 見下表。
>
...
| K-4 | G-L2 拍板 | 刪 `lib/games/hub-progress.ts`＋測試，不做貼紙／任務／動線提示 | S | ✅ `03c8ac20` |
...
| ID | 項目 | 狀態 |
|----|------|------|
| K-13 | 結算站：下一關＝往前箭頭、換一張＝翻頁、再玩＝循環箭頭；去下一站＝下一款縮圖＋小箭頭 | ✅ `79e38d9d` |
| K-14 | hub 卡片：56px 玩法 play 鈕（蠟筆／交換／方塊落下）、首張呼吸、三字動作詞＋圖示、meta 只留年齡；FAB 夜間保持淺底 | ✅ `79e38d9d` |
| K-15 | 方塊 ready 面純 CSS 無字玩法示範（掉進缺口→整列亮→消掉） | ✅ `79e38d9d` |
| K-16 | 方塊 over 層夜間標題白字壓奶油底（釘回馬卡龍 --ink）；就寢時段 night 局內 .playArea 內距 12→4 保 ≥25px 格子 | ✅ `79e38d9d` |
