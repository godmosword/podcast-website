...
| ID | 說明 |
|----|------|
| content(subtitles): ep-24 字幕校對 + `--mark`（重轉錄補尾段、刪幻覺／重複） | 待 commit |
| content(characters): ep-24 hero 定裝「小紅賽車年幼版」「小紅賽車的爸爸年輕版」（API 2 次） | 待 commit |
...
| 步驟 | 狀態 | 備註 |
|------|------|------|
| 字幕校對 + `--mark` | ✅ | 134→133 句；整集重轉錄補尾段；刪害羞重複句／「第一次發現」；修找找看／她／語病 → `_proofread/ep-24.json`（待 commit） |
| 定裝照（年齡變體） | ✅ | hero isolate：小紅賽車年幼版（青綠奶嘴）、小紅賽車的爸爸年輕版（翹鬍子／號碼 1）；API 2 次（待 commit） |
| `illustrate --segment-only` → 生圖 → `--approve` | ⬚ | 待做（需已 `--mark`） |
