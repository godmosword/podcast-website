...
describe("playground detail route helpers", () => {
  ...
  it("keeps current IDs unique and produces the canonical path", () => {
    ...
    expect(places).toHaveLength(99);
    expect(new Set(ids).size).toBe(99);
    expect(playgroundDetailPath("ty-kids-museum")).toBe(
      "/for-parents/play-map/ty-kids-museum",
    );
    expect(playgroundDetailPath("kh-family-playground")).toBe(
      "/for-parents/play-map/kh-family-playground",
    );
  });

  it("reserves collections under /play-map", () => {
    expect(RESERVED_PLAY_MAP_SEGMENTS.has("collections")).toBe(true);
    expect([...RESERVED_PLAY_MAP_SEGMENTS]).toEqual(["collections"]);
  });
