...
/** 將執行期關卡序列化回 JSON（除錯／匯出用）。 */
export function levelToJson(
  lv: AdventureLevel,
  tileSize: number,
): AdventureLevelJson {
  const solid: string[] = [...lv.solid];
  const spikes: string[] = [...lv.spikes];
  const coins: [number, number][] = lv.coins.map((c) => [
    Math.floor(c.x / tileSize),
    Math.floor(c.y / tileSize),
  ]);
  const enemies = lv.enemies.map((e) => ({
    x: Math.floor(e.x / tileSize),
    y: Math.floor((e.y + e.h) / tileSize) - 1,
    vx: e.vx,
  }));
  return {
    id: lv.id,
    ...
    enemies,
    start: [
      Math.floor(lv.start.x / tileSize),
      Math.floor((lv.start.y + 2) / tileSize),
    ],
    finish: [
      Math.floor(lv.finish.x / tileSize),
      Math.floor(lv.finish.y / tileSize),
      Math.round(lv.finish.w / tileSize),
      Math.round(lv.finish.h / tileSize),
    ],
  };
}
