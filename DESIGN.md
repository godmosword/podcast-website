...
- **留言牆（`/feedback`）**：維持 640 單欄、不掛 `SiteHeader`。首屏是馬米定裝身份層（`public/characters/馬米.jpg`，約 120–160px 黏土相框）＋對話泡泡邀請。頁底 `--page-warm-*`。表單**一律**在初始 HTML：信紙面（左緣 2px 暖色尺線）＋標題「寫給馬米」＋蜜罐 `website`＋可見「暱稱／信箱（選填）／留言」標籤與雙同意；留言欄底為實色、不畫橫線。無 `DATABASE_URL` 或送出回 unavailable 時，表單仍在，mailto「用 email 留言」只當次要備援（可帶入已填暱稱／正文）。牆用 Suspense；空牆只留 CTA 指向 `#feedback-message`，不放範例卡、不寫「還沒有公開留言」；有核准留言即列牆。真留言 `--hairline` 列。夜間圓章 `--c-*` 混色約 40%。蜜罐移出畫面，不用 `display: none`。
