...
- **留言牆（`/feedback`）**：維持 640 單欄、不掛 `SiteHeader`。首屏是馬米定裝身份層（`public/characters/馬米.jpg`，約 120–160px 黏土相框）＋對話泡泡邀請；「給家長」與審核句在同意列附近，不佔 H1 上方。頁底 `--page-warm-*`。表單**一律**在初始 HTML：信紙面（左緣 2px 暖色尺線）＋標題「寫給馬米」＋蜜罐 `website`＋起頭 chip＋可送出欄位。無 `DATABASE_URL` 或送出回 unavailable 時，表單仍在，mailto「用 email 留言」只當次要備援（可帶入已填暱稱／正文）。牆用 Suspense；核准少於 3 則只示範卡（實線暖面、非虛線），不寫「還沒有公開留言」；空牆 CTA 恆指向 `#feedback-message`。真留言 `--hairline` 列。夜間圓章 `--c-*` 混色約 40%。蜜罐移出畫面，不用 `display: none`。
...
1. **SiteNavBar**（全站橘色頂欄 + 訂閱 CTA）
   ...
   - **≥980px 懸浮膠囊**：外層 `.bar` 透明，`.inner` 改懸浮玻璃膠囊（`max-width: 960px`、高 56px）；**已移除 `.desktopNav`**——兒童三入口不再佔頂欄主列，改由漢堡抽屜承接。家長取向的親子指南／親子景點收進抽屜；角色圖鑑與繪本著色亦在抽屜。無「更多」下拉。Threads 育兒分享仍由 `/for-parents` 頁內「育兒小筆記」外連卡承接。**成長主題（`/topic`）不佔導覽**，頁面仍可直達。
   ...
   - **著色本頁 active 態**：抽屜用完整 `internalHrefs` 做**最長匹配獨佔**，高亮「繪本著色」。不含搜尋列（故事搜尋仍在 `/stories`）。「親子景點」連 `/for-parents/play-map`，與「親子指南」並列於家長組。不列「主題分類」。繪本著色雖為 `/games` 子路徑，仍在探索組獨立列出（兒童動線不應只能從遊樂園內層進入）；`/for-parents/play-map` 僅「親子景點」高亮。
...
5. **頁尾 snap pane**（`#landing-foot`）：只承 `SiteFooter`，**不新增第五個 snap 段**。原 `ExploreGrid`（「都去哪裡玩？」地圖大卡＋六格磁貼）與漢堡抽屜 7 列完全重疊，**已刪**；入口改由漢堡抽屜承接。`.footerPane` 維持至少一屏、`flex-start`＋頁尾 `margin-top: auto` 貼底；暖色漸層橋接在 pane 頂緣。≤768px 加 `padding-bottom: calc(44px + var(--safe-bottom))` 為貼底 `SegmentNav` 留位。
