...
- **親子景點頁（`/for-parents/play-map`）**：不掛 `SiteHeader`（全域 `SiteNavBar` 已顯示品牌，再放一次會出現兩個字標與重複 h1）；工具殼無行銷 hero／長 lede；H1「親子遊樂地圖」在工具頂列；瀏覽互斥為 **卡片｜地圖**（卡片殼可 SSR、先可用；Leaflet 僅地圖 tab 動態載入）；意圖／縣市／類型用 chip 篩選；有 `imageSrc` 顯示實景縮圖，無圖時以類型色條與短標籤構成卡片
- 桌面端維持單欄，兩側留白
