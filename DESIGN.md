...
1. **SiteNavBar**（全站橘色頂欄 + 訂閱 CTA）
   - **頂欄常駐列（2026-08-30 同構，CRITICAL-1=B／2=A）**：`.inner` 用 `justify-content: flex-start`；右側 `.actions` 用 `margin-left: auto` 推至最右。**兩斷點同構**——**所有寬度**皆為 `[品牌] [選單] …… [首頁] [訂閱] [留言]`（漢堡緊接品牌右側、`.actions` 在最右，維持現狀）。品牌 pill **即首頁入口**（連 `/`），**`.actions` 內另列「首頁」文字連結**（與品牌互補；**桌面也顯示**）。**選單觸發器**緊接品牌右側、**所有寬度都在**（桌面不得 `display: none`）。右側 **`.actions` 常用組**包 `[首頁]／訂閱／留言`，`role="group"`、`aria-label="常用"`。**主題切換移出頂欄**，改在抽屜底部。訂閱文案「**訂閱**」；`visiblePlatforms()` 為空時**不得整顆消失**，退為站內 `/subscribe`。
