...
- **親子景點頁（`/for-parents/play-map`）**：不掛 `SiteHeader`（全域 `SiteNavBar` 已顯示品牌，再放一次會出現兩個字標與重複 h1）；工具殼無行銷 hero／長 lede；H1「親子遊樂地圖」在工具頂列。**手機**名單優先（Leaflet 僅「看地圖」後動態載入；返回名單不重建容器）；地圖模式全幅，篩選列留在名單，沒有 bottom sheet snap。全國未定位時依地圖寬度 `setView`（西緣釘在台灣海峽東側，寬圖多餘經度給太平洋），不把福建當主畫面、不撒全部針。**桌面 ≥980px** 名單與地圖並排（名單略增至約 44%），不顯示「看地圖」。一列主控制（附近／雨天／免費／放電／室內＋次要「篩選」）；縣市／類型收進可收合篩選面板。無場館照片時以**類型縮圖 plate**（7 種手繪 SVG，置左）與家長筆記構成卡片；Google 導航／顯示位置用頁面場館名＋縣市，不用 lat,lng 圖釘。頁面層篩選 chip 走 ghost／`--accent-ink`；`--map-chip*` 只留給地圖 overlay（縮放鍵、cluster、mapHint）。**七類型母題有兩個消費點**：卡片 plate（彩色，96 viewBox，`components/for-parents/type-scenes/`）與**地圖針中心剪影**（單色 `currentColor`，24 viewBox，`lib/playground-type-glyph.ts`）。針用圓形黏土容器＋較大剪影（色相＋形狀雙重編碼）。剪影色為**固定美術色** `#34302b`（不隨主題翻轉）；「其他」類的針與卡片 accent 用固定淺沙 `#cfcac2`，不用 `--ink-soft`。針**不**共用 `--map-chip*`。
- 桌面端維持單欄，兩側留白
