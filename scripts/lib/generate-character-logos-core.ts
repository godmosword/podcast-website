...
export function contrastFailureLines(
  ...
  return targets.flatMap((logo) => {
    ...
    return [
      `${logo.slug} sil=${audit.silhouette.toFixed(2)} face=${audit.face.toFixed(2)} gate=${audit.gate.toFixed(1)} margin=${audit.margin.toFixed(2)} hueDist=${audit.hueDist.toFixed(1)}`,
    ];
  });
}
