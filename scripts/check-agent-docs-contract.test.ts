...
describe("agent docs routing contract", () => {
  it("Cursor active 路由使用 gpt-5.6-luna-max-fast 與 grok-4.5-fast-high", () => {
    for (const file of CURSOR_ACTIVE_FILES) {
      const text = readRepoFile(file);
      expect(text, `${file} 須含 Luna slug`).toContain("gpt-5.6-luna-max-fast");
      expect(text, `${file} 須含 Grok High slug`).toContain("grok-4.5-fast-high");
    }

    const workflow = readRepoFile(CURSOR_META);
    expect(workflow).toContain("gpt-5.6-luna-max-fast");
    expect(workflow).toContain("grok-4.5-fast-high");
  });

  it("active 路由不得使用已淘汰的 grok-4.5-fast-medium slug", () => {
  ...
  it("不得把 Luna slug 寫進 codex exec", () => {
    ...
    for (const text of [claudePlan, claudeAction, failures]) {
      expect(text).not.toContain("codex exec -m gpt-5.6-luna");
      expect(text).not.toMatch(/codex exec -m gpt-5\.6-luna-max-fast/);
    }

    expect(claudePlan).toContain("codex exec -m gpt-5.6");
    expect(claudeAction).toContain("codex exec -m gpt-5.6");
  });
  ...
  it("AGENT-FAILURES 探活表使用 grok-4.5-fast-high", () => {
    const failures = readRepoFile(FAILURES);
    const probeSection =
      failures.match(/## 探活命令[\s\S]*?(?=## 已知案例)/)?.[0] ?? "";
    expect(probeSection.length).toBeGreaterThan(0);
    expect(probeSection).toContain("grok-4.5-fast-high");
