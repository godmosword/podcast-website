...
- **GEO live 煙霧 vs MVP 無字幕**：`verify:geo-live` 不再要求最新集一定有 `transcript.vtt` 200。無 `data/subtitles` 側車時改確認 RSS／JSON-LD **未**宣告逐字稿，且 VTT 為 404（ep-27 本機 sync 未跑 Whisper）。有側車仍走原契約。**未改** Apple sync workflow。
