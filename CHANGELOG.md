...
- **美術審 Batch B（M4／M5／M6／M8／L2／L6）**：≤480 故事列表縮圖 88 → 120px、摘要 3 → 2 行（`STORIES_CATALOG_COVER_SIZES` 同步）；相關故事／收藏／主題／車種列表卡一律 `hideMeta`，與 /stories 目錄卡同一種密度；播放器主播放鈕新增 `--play-size`（72／76／80）比鄰鈕大一階、`--controls-block` 跟著 +12，字級鈕 `text-size` 圖示從「A I」改成大 A 小 a；`/subscribe` 登記關閉時把四個平台做成大按鈕列（`subscribe-closed` 點擊來源、`utm_medium=subscribe_cta`），不再只剩一句提示；角色圖鑑定裝照日間 `mix-blend-mode: multiply` 讓棚拍奶油白等於底墊色、兩主題疊邊緣暈影（夜間加重成聚光）——不重拍、不生圖；`/about` 標題下放馬米＆Bonbon 圓形頭像（既有 `public/characters/` 素材，`transform-origin` 對臉）。重錄 10 張 Darwin 基線（about ×4、characters ×4、subscribe 390 ×2）。codex readonly 審兩次超時，Leader 自審（記於 AGENT-FAILURES）。**未改** Apple sync workflow、`useMapCamera`／`ZoneSheet`、`PlayButton`、字幕邏輯。
- **美術審 Batch A（M2／M7／L1／L3／L4／L5）**：SiteHeader hero 夜間補 `--bedtime-veil` 漸層（同 Landing 的 scrim 配方，上 38%／中 22%／底 12%，不做均勻降亮度）；家長頁育兒小筆記拿掉左緣色條，改 hairline＋`--surface-elevated`＋`--elev-1`，統計格刪「同步檢查」剩三格（手機一列三格、桌機右欄直向堆疊），FAQ「3–6 歲」改「3–7」；頁尾 ConnectHub ≤480 改 4 欄 grid（不再 3+1 折行）、法務連結前的分隔點改 `::before`（≤480 自成一行時不畫）；故事卡 EP chip 固定 `.marker-lilac` 不再隨 `story.color`、遊戲卡封面角標年齡拿掉只留 meta 列；`data/stories.ts` `ageRange` 統一「約 3–7 歲」／「約 4–7 歲」、標籤「勇敢」併入「勇氣」（`verify:episodes` 錯誤 0）；`.site-root` 改 `min-height: calc(100% - var(--nav-h))`，body 不再比視窗高 64px（水合前點圓鈕不再推頁）。重錄 17 張 Darwin 基線（home／about／characters／feedback／subscribe 390 ×2、for-parents ×5、stories night ×2）。**未改** Apple sync workflow、`useMapCamera`／`ZoneSheet`、`PlayButton`、`data/browse-index.json`（prebuild 產物）。
...
- **Hero 小紅補上車門號碼「2」，並修好車頭零件的錨定**：`build.py` 補上白圓底藍字的車門號碼「2」——canon 的主要自有識別，圓底做成扁球而非平面圓盤才貼得住 `.26` bevel 的圓角側面。微笑線由線徑 `.026` 加粗到 `.036`、嘴角由 ±`.17` 拉到 ±`.21`，不再遠看塌成一根橫桿。同時修好前一版縮短車身留下的破綻：`Body` 的 Y 從 `1.65` 縮到 `1.52` 時，車頭面由 `-.825` 退到 `-.760`，但 `Bumper`／`Headlight`／`Smile`／`Hood stripe` 仍釘在舊車頭座標，Cycles 預覽可見微笑線是一條懸空的深色線、黃大燈是浮在車頭前的球；四者一律 +`.065` 重新錨定，嘴角埋入車頭、嘴心落在保險桿前緣。輪徑刻意不動——`WHEEL_RADIUS` 同時是 `config.ts` 裡`wheelAnimationTime()` 的換算基準，改動會讓輪子轉速與路面前進距離脫鉤。GLB／poster／manifest 同輪重建，`little-red` 4,924 三角面、全場 17,557（預算 40,000），`validate:hero-world` 全通過。**未改** Apple sync workflow。
- **小紅車殼與世界打光對齊定裝照**：`build.py` 原本讓小紅的車殼與書封屋頂共用同一個赭紅 `red`，實機上車子讀成鮭魚色，跟平面 hero 圖認不出是同一台車。新增 `car_red`（飽和正紅）／`car_white`／`car_eye` 三個色票，車身依定裝照比例縮短加高（`1.03×1.65×0.51` → `1.06×1.52×0.56`）、bevel 加大，眼睛從 Y 厚 `.035` 的貼紙改成真的突出來的立體眼。`WORLD_ROUGHNESS` 從「七個值擠在 .58–.96、其餘一律 .86」拆成軟陶土／粗糙地面／玻璃三段；新增 `WorldEnvironment.tsx` 程序式環境漸層（`envIntensity .38`）與不投影的逆光輪廓光，roughness 的差異才看得出來。`shadow-mapSize` 改由 quality tier 決定，high 升到 2048（1024 在 1.5 DPR 下屋簷與車底的陰影邊緣看得出鋸齒）。GLB／poster／manifest 已用 Blender 同輪重建，`buildScriptSha256` 與 `build.py` 一致。**未改** Apple sync workflow、`useMapCamera`／`ZoneSheet`。
...
- **標題列去框＋字級收斂＋夜間抽屜暖化（2026-08-31）**：
  ...
  實測（真實合成像素）：面板正文 12.33:1、「給爸媽」小標 7.34:1、8 個寬度（360–1280）無溢出且觸控區維持 44px。
  4. **訂閱下拉同步暖化**——`.dropdown` 夜間亦改 `--nav-panel-bg`。它與抽屜同為頂欄浮層，抽屜暖化後若留在 `--card`(#2c3450 靛藍) 會出現「同一列兩個浮層兩種底色」。此處**快照拍不到**（要點開才出現），故補 CSS 契約測試。
...
- **`/adventures` 首訪提示在手機完全看不見（桌機／手機對等稽核）**：`.tapHint`（`z-index: 4`）在 ≤480px 被底部三個常駐浮層同時遮住——`IslandPickerStrip`（z 6）、`KidsPlayDock`（z 15，`data-lift="picker"` 抬到 92px）、`MapControls`（184px 高的直排，z 5）；且 `.tapHintText` 是 `white-space: nowrap`，整條固定 459px 寬，在 360–390px 視窗左右各溢出 34–49px、文字左緣被切 14–29px。**改為 ≤480 錨在天象帶之下**（`top: calc(var(--sky-top) + var(--sky-size) + 24px)`、左錨 `left: calc(12px + var(--safe-left))`），不動 `z-index`——`tapHint(4) < MapControls(5) < strip(6)` 是刻意分層，且抬高多少都仍被 dock（z 15）蓋住，`map-camera-utils.ts` 的 `fitAvailableViewport` 也已把底部 148px 寫進相機保留區。加 `max-width` 夾制（含 safe inset，**不用 `100vw`**）、入場動畫方向跟著翻轉（水平位移走 `--tap-hint-x` 變數，`@keyframes` 與 reduced-motion 共用同一式子，毋須複製）。三個容易踩空的細節：(1) **不可加 `var(--safe-top)`**——`--nav-h` 已含一次而 `.map` 在頂欄之下，再加一次在瀏海機上會把提示推低 47–59px（實測注入 `--safe-top: 59px` 時距 map 頂變成 71px 而非 12px），桌機模擬永遠看不出來；(2) `max-width` 若配 `white-space: nowrap` **等於白設**，flex item 的 `min-width: auto` 會被 min-content 撐住，故 ≤480 一併改 `normal`；(3) 覆寫必須是**獨立且排在 `.tapHint` 基礎規則之後**的 480 區塊——媒體查詢不加權重，寫進本檔上方那個 480 區塊會被下方基礎規則靜默蓋回去（已加契約測試守住順序）。日／月是 `right` 定位、視窗一窄就左移，故提示錨在天象帶之下而非單純置頂：移動只活 8 秒的提示，不動永久美術。色票 `#fff8ea`／`#f0b64e`／`#7a5410` 一併改走 `--map-chip`／`--map-chip-line`／`--map-chip-ink`（`#7a5410` 與 token 逐字相同，DESIGN §101／§227），並補上 `MapControls` 那組深藍外陰影，夜間才與旁邊的控制鈕同一套語彙。
  **文案縮短為「點一座島，飛過去玩」**（原「點一座島飛過去；要看故事／探索點可點「來這裡逛逛」」）：後半句預告的召喚把手依 DESIGN §245 是**進島之後**才出現的控制項，在世界層預告看不見的東西對家長是雜訊、對兒童是零；`UniverseMap.tsx` 的 sr-only 導覽段落保留完整說明，AT 資訊不流失。⚠️ 這**解除了一條既有契約**——`e2e/smoke.spec.ts` 與 `e2e/universe-map.spec.ts` 原本各自明文斷言「提示必須提到『來這裡逛逛』」，是刻意寫的，不是誤刪；改回去前請先讀本段。
  CI 之所以沒擋下：`e2e/universe-map.spec.ts` 的 `openMap()` 預設 **1280×800**，三個既有 tap hint 測試全跑桌機寬，而島選擇列只在 ≤480 渲染；加上 Playwright 的 `toBeVisible()` **不偵測遮蔽**，即使改跑 390px 也照樣綠。已補 320／360／375／390 的幾何測試（`elementFromPoint` 遮蔽偵測＋與三個浮層的矩形不重疊＋邊距 ≥16px）。**未改** Apple sync workflow。

- **頂欄訂閱下拉在窄屏左緣溢出畫面（全站）**：`.wrap { position: relative }` 讓 240px 的 `.dropdown` 錨定在只有 67px 寬的觸發鍵上，左緣在 390／375／360 分別跑到 −23／−31／−38px，左圓角被切、貼齊螢幕邊。頁面 `scrollWidth` 不變（靜默裁切），所以橫向溢出偵測抓不到；`min-width: min(240px, calc(100vw - 24px))` 也攔不住——360px 時 `100vw−24px` 是 336 > 240，那條要 264px 以下才生效，真正的問題是**錨點**不是寬度。**≤480 改讓 `.wrap` 轉 `position: static`**，下拉落到 `SiteNavBar` `.inner` 的 containing block（`container-type: inline-size` 對所有後代生效，故**毋須搬 DOM**，`wrapRef.contains()` 的外部點擊判定不受影響）。`right` 顯式寫 `calc(16px + var(--safe-right))`——containing block 是 padding box，`right: 0` 會貼死螢幕右緣（同 `.panel` 在 <980 變滿版的成因）。**只限 ≤480**：≥980 時 `.inner` 是 960px 膠囊、`.panel` 是 `right: 16px`，全域套用會讓訂閱下拉開在與漢堡抽屜完全相同的位置。夜間同底契約（`--nav-panel-bg`）原封保留。新增 `e2e/nav-subscribe.spec.ts` 涵蓋 240／360／375／390／480／481／768／979／980／1280 的幾何，以及 Esc／點外部／焦點歸還／單一浮層等互動契約。**未改** Apple sync workflow。
