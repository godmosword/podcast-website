...
- **樂園地圖漫遊者 PNG 資產（R-anim 1.5 資產）**：`public/adventures/roamers/xiao-hong.png`、`duo-duo.png`（gpt-image-2 edit + 定裝照 reference、magenta chroma-key、sharp 後製 ~728px RGBA、底中心錨點）；生圖腳本 `scripts/generate-roamer-assets.ts`（staging → contact sheet → `--approve`）；`MAP_ROAMERS` 啟用小紅＋多多（`carpark-loop`、`startOffset` 0／0.5）
