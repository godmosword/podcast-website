...
家族：construction · 車種：水泥車 · 特徵：滾筒 · Tier 2
...
Subject: 阿尼 the 水泥車, reduced to one continuous rounded silhouette with exactly one defining feature: 滾筒. Front three-quarter-low view; the vehicle's front IS the face — grille as mouth, headlights as eyes.
...
Color: exactly three semantic colors total — two IP base colors (#EF9F05 primary, #D4EEF4 secondary) plus the background. Keep the secondary color as one large continuous region. Silhouette-to-background contrast uses the hue-weighted gate (2.8 / 3.6 / 4.5) with margin >= 0.2; facial marks on the lighter IP region >= 5:1.
...
Asset paths: public/characters/logo/a-ni-512.webp, a-ni-128.webp, a-ni-32.webp.
```
