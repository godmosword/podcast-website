...
家族：transit · 車種：計程車 · 特徵：車頂燈箱 · Tier 2
...
Subject: 計程 the 計程車, reduced to one continuous rounded silhouette with exactly one defining feature: 車頂燈箱. Front three-quarter-low view; the vehicle's front IS the face — grille as mouth, headlights as eyes.
...
Color: exactly three semantic colors total — two IP base colors (#FDD438 primary, #F2E4CC secondary) plus the background. Keep the secondary color as one large continuous region. Silhouette-to-background contrast uses the hue-weighted gate (2.8 / 3.6 / 4.5) with margin >= 0.2; facial marks on the lighter IP region >= 5:1.
...
Asset paths: public/characters/logo/huang-ji-cheng-512.webp, huang-ji-cheng-128.webp, huang-ji-cheng-32.webp.
```
