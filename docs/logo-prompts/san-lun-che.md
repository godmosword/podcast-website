...
家族：transit · 車種：三輪車 · 特徵：單前輪 · Tier 2
...
Subject: 三輪 the 三輪車, reduced to one continuous rounded silhouette with exactly one defining feature: 單前輪. Front three-quarter-low view; the vehicle's front IS the face — grille as mouth, headlights as eyes.
...
Color: exactly three semantic colors total — two IP base colors (#F5C155 primary, #F8C8C8 secondary) plus the background. Keep the secondary color as one large continuous region. Silhouette-to-background contrast uses the hue-weighted gate (2.8 / 3.6 / 4.5) with margin >= 0.2; facial marks on the lighter IP region >= 5:1.
...
Asset paths: public/characters/logo/san-lun-che-512.webp, san-lun-che-128.webp, san-lun-che-32.webp.
```
