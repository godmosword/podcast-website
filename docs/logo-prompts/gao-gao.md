...
Subject: 高高 the 摩天輪, reduced to one continuous rounded silhouette with exactly one defining feature: 輪圈輻條. Front three-quarter-low view; the vehicle's front IS the face — grille as mouth, headlights as eyes.
...
Color: exactly three semantic colors total — two IP base colors (#2A6CB0 primary, #AB8400 secondary) plus the background. Keep the secondary color as one large continuous region. Silhouette-to-background contrast uses the hue-weighted gate (2.8 / 3.6 / 4.5) with margin >= 0.2; secondary-to-background >= 3:1; facial marks on the lighter IP region >= 5:1.
...
Asset paths: public/characters/logo/gao-gao-512.webp, gao-gao-128.webp, gao-gao-32.webp.
```
