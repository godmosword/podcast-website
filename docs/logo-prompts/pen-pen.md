...
Subject: 噴噴 the 消毒車, reduced to one continuous rounded silhouette with exactly one defining feature: 噴嘴. Front three-quarter-low view; the vehicle's front IS the face — grille as mouth, headlights as eyes.
...
Color: exactly three semantic colors total — two IP base colors (#E7D3B7 primary, #91C6D4 secondary) plus the background. Keep the secondary color as one large continuous region. Silhouette-to-background contrast uses the hue-weighted gate (2.8 / 3.6 / 4.5) with margin >= 0.2; facial marks on the lighter IP region >= 5:1.
...
Asset paths: public/characters/logo/pen-pen-512.webp, pen-pen-128.webp, pen-pen-32.webp.
```
