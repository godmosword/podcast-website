...
家族：fantasy · 車種：海龜 · 特徵：龜殼 · Pilot · Tier 1
...
Subject: 暖暖 the 海龜, reduced to one continuous rounded silhouette with exactly one defining feature: 龜殼.
...
Color: exactly three semantic colors total — two IP base colors (#F5A882 primary, #F8E8D0 secondary) plus the background. Keep the secondary color as one large continuous region. Silhouette-to-background contrast uses the hue-weighted gate (2.8 / 3.6 / 4.5) with margin >= 0.2; facial marks on the lighter IP region >= 5:1.
...
Asset paths: public/characters/logo/nuan-nuan-turtle-512.webp, nuan-nuan-turtle-128.webp, nuan-nuan-turtle-32.webp.
```
