...
家族：construction · 車種：清潔車 · 特徵：旋轉刷 · Tier 2
...
Subject: 玲玲 the 清潔車, reduced to one continuous rounded silhouette with exactly one defining feature: 旋轉刷. Front three-quarter-low view; the vehicle's front IS the face — grille as mouth, headlights as eyes.
...
Color: exactly three semantic colors total — two IP base colors (#A8E86B primary, #F3E6D0 secondary) plus the background. Keep the secondary color as one large continuous region. Silhouette-to-background contrast uses the hue-weighted gate (2.8 / 3.6 / 4.5) with margin >= 0.2; facial marks on the lighter IP region >= 5:1.
...
Asset paths: public/characters/logo/ling-ling-512.webp, ling-ling-128.webp, ling-ling-32.webp.
```
