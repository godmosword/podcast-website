...
Background: fully opaque edge-to-edge solid OKLCH(L 0.62 C 0.12 H 75) / #A8783A (construction family, 工程建設). Use this color for the background only. Background stays visually flat: no vignette, spotlight, or directional wash.
...
Color: exactly three semantic colors total — two IP base colors (#FFE566 primary, #D4EEF4 secondary) plus the background. Keep the secondary color as one large continuous region. Silhouette-to-background contrast >= 3:1; facial marks on the lighter IP region >= 4.5:1.
