...
Background: fully opaque edge-to-edge solid OKLCH(L 0.45 C 0.08 H 70) / #714D1D (construction family, 工程建設). Use this color for the background only. Background stays visually flat: no vignette, spotlight, or directional wash.
...
Color: exactly three semantic colors total — two IP base colors (#F7A921 primary, #D4EEF4 secondary) plus the background. Keep the secondary color as one large continuous region. Silhouette-to-background contrast >= 3.6:1 with margin >= 0.2; facial marks on the lighter IP region >= 5:1.
