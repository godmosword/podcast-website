...
家族：transit · 車種：高鐵 · 特徵：流線頭錐 · Tier 2
...
Subject: 小橘 the 高鐵, reduced to one continuous rounded silhouette with exactly one defining feature: 流線頭錐. Front three-quarter-low view; the vehicle's front IS the face — grille as mouth, headlights as eyes.
...
Color: exactly three semantic colors total — two IP base colors (#EEDAC4 primary, #FFD4A0 secondary) plus the background. Keep the secondary color as one large continuous region. Silhouette-to-background contrast uses the hue-weighted gate (2.8 / 3.6 / 4.5) with margin >= 0.2; facial marks on the lighter IP region >= 5:1.
...
Asset paths: public/characters/logo/xiao-ju-hsr-512.webp, xiao-ju-hsr-128.webp, xiao-ju-hsr-32.webp.
```
