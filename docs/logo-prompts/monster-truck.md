...
家族：fantasy · 車種：怪獸卡車 · 特徵：巨型輪 · Tier 2
...
Subject: 萌萌 the 怪獸卡車, reduced to one continuous rounded silhouette with exactly one defining feature: 巨型輪.
...
Color: exactly three semantic colors total — two IP base colors (#ECAE29 primary, #3A342E secondary) plus the background. Keep the secondary color as one large continuous region. Silhouette-to-background contrast uses the hue-weighted gate (2.8 / 3.6 / 4.5) with margin >= 0.2; facial marks on the lighter IP region >= 5:1.
...
Asset paths: public/characters/logo/monster-truck-512.webp, monster-truck-128.webp, monster-truck-32.webp.
```
