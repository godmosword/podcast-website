...
家族：transit · 車種：自駕計程車 · 特徵：車頂 LiDAR 圓罩 · Tier 2
...
Subject: 知知 the 自駕計程車, reduced to one continuous rounded silhouette with exactly one defining feature: 車頂 LiDAR 圓罩. Front three-quarter-low view; the vehicle's front IS the face — grille as mouth, headlights as eyes.
...
Color: exactly three semantic colors total — two IP base colors (#E8EEF2 primary, #7EF0FF secondary) plus the background. Keep the secondary color as one large continuous region. Silhouette-to-background contrast uses the hue-weighted gate (2.8 / 3.6 / 4.5) with margin >= 0.2; facial marks on the lighter IP region >= 5:1.
...
Asset paths: public/characters/logo/zhi-zhi-512.webp, zhi-zhi-128.webp, zhi-zhi-32.webp.
```
