...
Subject: 安安 the 救護車, reduced to one continuous rounded silhouette with exactly one defining feature: 十字. Front three-quarter-low view; the vehicle's front IS the face — grille as mouth, headlights as eyes.
...
Color: exactly three semantic colors total — two IP base colors (#F0DCC4 primary, #E52818 secondary) plus the background. Keep the secondary color as one large continuous region. Silhouette-to-background contrast uses the hue-weighted gate (2.8 / 3.6 / 4.5) with margin >= 0.2; facial marks on the lighter IP region >= 5:1.
...
Asset paths: public/characters/logo/an-an-512.webp, an-an-128.webp, an-an-32.webp.
```
