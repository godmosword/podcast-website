...
家族：rescue · 車種：警車 · 特徵：車頂警示燈條 · Tier 1
...
Subject: 亮亮 the 警車, reduced to one continuous rounded silhouette with exactly one defining feature: 車頂警示燈條. Front three-quarter-low view; the vehicle's front IS the face — grille as mouth, headlights as eyes.
...
Color: exactly three semantic colors total — two IP base colors (#E8EEF4 primary, #E8C43A secondary) plus the background. Keep the secondary color as one large continuous region. Silhouette-to-background contrast uses the hue-weighted gate (2.8 / 3.6 / 4.5) with margin >= 0.2; facial marks on the lighter IP region >= 5:1.
...
Asset paths: public/characters/logo/liang-liang-512.webp, liang-liang-128.webp, liang-liang-32.webp.
```
