...
| 對象 | 最低比 | 量測 |
|---|---|---|
| 剪影主體（通常是 primary）對家族背景 | hueDist ≥ 60° → 2.8:1；30–60° → 3.6:1；＜ 30° → 4.5:1。margin ≥ 0.2 | WCAG 相對亮度，門檻隨 OKLCH 色相距離加權。背景不得呼應成員識別色相 |
| 次色大面積對家族背景 | ≥ 3:1 | WCAG。`joy` 禁止用白／米色窗帶充當次色；暗底家族不得用溶進背景的深褐／黑 |
| 臉部小標記（眼、嘴）對其下方表面 | ≥ 5.0:1，faceMargin ≥ 0.2 | 眼睛落在兩個 IP 色中較亮的那塊 |
