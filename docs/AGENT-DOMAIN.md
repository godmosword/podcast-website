...
| **視覺回歸（改樣式／版面時）** | `npm run test:visual:trusted`（**本機 pre-push 工具，刻意不進 CI**：baseline 為 `-chromium-darwin`，CI 跑 ubuntu，像素不可能相符）。trusted 會以 `VISUAL_FIXTURE=1` 重建 SSG，截凍結子集。預設 `npm run test:visual` 會 skip，**skip ≠ 通過**；重產須在 **darwin** 加 `-- --update-snapshots` 並逐張人工目檢。禁止在 Vercel 設 `VISUAL_FIXTURE`／`NEXT_PUBLIC_VISUAL_FIXTURE` |
...
| 2026-08-26 | VIS-DEBT-2／VIS-GAP-1：視覺 fixture 凍結資料 + 兒童主路徑 baseline；trusted 必須 `VISUAL_FIXTURE=1` 重建 |
