...
| `coverageNote`（選填） | **資料涵蓋範圍或可信度的誠實聲明**。不是遊玩建議（那是 `tips`）。無值則不渲染。**版位：** full variant 在 Tips 之後、收費／來源之前；compact variant 沒有 Tips，落在地址與操作按鈕之間。「導航指向鄰近地標而非場館本身」屬本欄（例：`hc-nanliao` 導航落點為南寮漁港旅遊服務中心），不寫進 `tips`。其他例：幼童遊具待現場確認（`hcx-hukou-sports`）、跨區園區的導航入口（`nt-metro-park`） |
