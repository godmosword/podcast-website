...
## `tips` 品質標準（取代已廢除的字數門檻）

`tips` 是「帶小孩時的實用提醒」。判準只有一條：

> **每一筆至少要有一件「不到現場不會知道、且只適用於這個場館」的事實。**

可用的事實類型：休館日／最後入園或售票時間／中午清館／需預約或分場次／
分區另計票／免費年齡或身高門檻／無遮蔭／館際距離與推車地形／身高限制。

**禁止：**

1. **設施列舉尾句**（「場內有 X、Y、Z。」）。與 `facilities` 重複，卡片層的
   `composeParentBlurb` 本來就會過濾。
2. **通用句。** 判準：把場館名換成任何其他場館後這句話依然成立，就是通用句
   （「假日人潮多，建議提早到」「記得帶水」）。
3. **資料可信度聲明。** 那是 `coverageNote`，不是 `tips`。
4. **內部編輯語言。** `feeNote`／`coverageNote` 會原樣渲染給家長，不得出現
   欄位名、「查無」、「待定稿」等寫給編輯看的字樣。

### 為什麼不用字數門檻

歷史上契約曾要求 `tips` 必須包含某個 `facilities`／`tags` 名稱，用意是逼出場館
專屬敘述。實際效果相反：作者寫不出自然帶到設施名的句子，就在句尾接
「場內有 X、Y、Z。」湊過去——73 筆全中，47 筆離開尾句就過不了契約。
契約自己製造了它要防的東西（見 commit `6a7d39f`）。

後續評估詳情頁「完備度 gate」時也重複踩到同一個坑：以 `tips` 字數 ≥25 當
分級門檻，實測門檻正好壓在中位數上，判別的是一兩個字的雜訊；而被篩掉的
清一色是免費公園——那不是品質差，是免費公園沒有票價分層、場次與休館日
可寫。**用可量測的代理指標當品質閘門，會篩出場館類型而不是品質，並誘導
補套話。**

因此：**不設字數門檻，也不用完備度分級決定詳情頁是否收錄。** 品質由上面
那條「至少一件現場才知道的事實」把關，靠人審，不靠數字。
...
- 真實地圖 overlay（縮放鍵／cluster／mapHint）可用 `--map-chip*`；頁面層 chip／Sheet **不**共用。OSM tile **禁止** invert（含夜間主題）
