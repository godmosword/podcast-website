...
Next.js 16 App Router、React 19、TypeScript strict、CSS Modules。首頁由 LandingHub、四個 LandingSegment、專用捲動容器、BedtimeLayer 和 DuduCompanion 組成（ADR-0004 移除了 SegmentNav 底列，換段改由 snap／方向鍵與各段可點的淡雙折線承擔）。全域 SiteNavBar 提供故事、遊戲、角色、宇宙地圖與家長景點；各功能頁直接以 App Router URL 進入。首頁保留原 metadata、canonical `/`、Podcast JSON-LD、可索引導言與 SSR 連結。故事播放有獨立沉浸式介面，音訊須由使用者啟動。Intro 不引入 Audio 或改寫播放器。
