...
2. **色彩校正（現行關閉）**：生圖給的是 `#a47846`，比 Art Bible 的步道色 `#d7c596` 更深更橘。**2026-09-11 裁決保留原色**——校正後路面偏淡、虛線對比跟著降，原色自己看更有份量；視差帶是獨立畫面，與地圖步道不同調可接受。機制保留（`RECOLOUR_STRENGTH = 0`），下面是校正曾經怎麼做的紀錄：
   ...
   - 強度 `0.78`，不做滿。做滿會把手捏質感洗平。校正後路面 `#cfb789`、虛線 `#f1e9cf`，
     對比仍足夠。
3. **裁到內容高度 ＋ 裁切 ＋ 交叉淡接**：原圖上下各有一大片透空，先裁到路面本身的高度（237px），前端排版要的是真實高度；再從「間隙中點」起裁、長度取整數個週期（實測 10 個 = 1630px），
